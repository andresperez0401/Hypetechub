...
export function LoginForm(): JSX.Element {
  const { loginLocal, isLoading, error, data } = useAuth();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="space-y-1">
        <label htmlFor="login-turnstile" className="block text-sm font-medium text-slate-700">
